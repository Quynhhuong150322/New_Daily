...
const HomeScreen = () => {
    ...
    useEffect(() => {
        const loadArticles = async () => {
            ...
            // Dựa trên thể loại hiện tại, lấy danh sách bài viết từ data.js hoặc API tương ứng
            if (currentCategory === 'all') {
                fetchedArticles = await fetchArticlesFromAPI('https://newsdata.io/api/1/news?country=vi&apikey=pub_35731380e0a51b24cc5e9e0c6dc8e7d1b73b8');
            } else if (currentCategory === 'sports') {
                fetchedArticles = SPORTS_API;
            } else if (currentCategory === 'technology') {
                fetchedArticles = TECHNOLOGY_API;
            } else if (currentCategory === 'WORLD_API') {
                fetchedArticles = worldData;
            } else {
                // Xử lý các thể loại khác ở đây
            }
