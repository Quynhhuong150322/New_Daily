...
const HomeScreen = () => {
    ...
    useEffect(() => {
        const loadArticles = async () => {
            ...
            // Determine the API URL based on the current category
            ...
            setFilteredArticles(fetchedArticles); // Lưu danh sách ban đầu
            setLoading(false);
        };
        loadArticles();
    }, [currentCategory]);

    // Hàm này sẽ được gọi khi thay đổi thể loại chọn
    const handleCategoryChange = (category) => {
        setCurrentCategory(category);
        // Lọc danh sách bài viết dựa trên thể loại chọn
        if (category === 'all') {
            setFilteredArticles(articles); // Hiển thị tất cả bài viết
        } else {
