...
const CarouselArticle = ({ item }) => {
    ...
    const newApiUrl = 'https://newsdata.io/api/1/news?country=vi&apikey=pub_35731380e0a51b24cc5e9e0c6dc8e7d1b73b8';

    const navigateToArticleDetail = async (article) => {
        const articles = await fetchArticles(newApiUrl); // Sử dụng API mới để lấy dữ liệu bài viết.
    };
...
const HomeScreen = () => {
    ...
    const [currentCategoryAPI, setCurrentCategoryAPI] = useState('https://newsdata.io/api/1/news?country=vi&apikey=pub_35731380e0a51b24cc5e9e0c6dc8e7d1b73b8');

    useEffect(() => {
        const loadArticles = async () => {
            const fetchedArticles = await fetchArticles(currentCategoryAPI);
