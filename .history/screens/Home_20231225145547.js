...
const HomeScreen = () => {
    ...
    // Tính toán index của Carousel dựa trên vị trí cuộn trang
        const { contentOffset } = event.nativeEvent;
        const index = Math.round(contentOffset.x / 400); // Giả sử mỗi mục trong Carousel có độ rộng 400
    ...
    return (
        ...
        <FlatList
            ...
            windowSize={21}
            horizontal // Đặt FlatList ngang để có thể cuộn trái phải
            onScroll={(event) => handleFlatListScroll(event)} // Bắt sự kiện cuộn trang
            pagingEnabled // Cho phép cuộn từng mục một
        />
        ...
        <Carousel
            ...
            sliderWidth={350}
            itemWidth={400}
            activeSlideAlignment="center" // Căn giữa mục được chọn
            firstItem={carouselIndex} // Đặt mục đầu tiên cho Carousel dựa trên index đã tính
        />
        </View>
    );
};
