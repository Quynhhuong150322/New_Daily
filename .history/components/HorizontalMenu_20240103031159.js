...
const HorizontalMenu = ({ currentCategory }) => {
    ...
    const loadData = async (category) => {
        ...
        if (category === 'Thế giới') {
            const fetchedArticles = await fetchArticles4();
            setArticles4(fetchedArticles);
        } else if (category === 'Thể thao') {
            const fetchedArticles = await fetchArticles5();
            setArticles4(fetchedArticles);
        } else if (category === 'Công nghệ') {
            const fetchedArticles = await fetchArticles6();
        } else if (category === 'Khoa học') {
            const fetchedArticles = await fetchArticles7();
        } else {
